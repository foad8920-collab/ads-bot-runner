...
async function warmupSession(page) {
    try {
        await logToDashboard(`☕ [Warm-up New Account Safety] تصفح أطول وتمرير عشوائي لحماية الحساب...`, 'info');
        ...
        await logToDashboard(`✅ تم إحماء الجلسة وتنويع السلوك للبوت الثاني بنجاح!`, 'success');
    } catch (e) {
...
async function processOnePostBot2(initialPostData) {
    ...
    // 💡 --- تأخير أمان متوسط لضمان أنه ينطلق بعد البوت الأول وقبل الثالث ---
    const initialOffsetDelay = randomDelay(240, 480); // تأخير بين 4 إلى 8 دقائق
    await logToDashboard(`⏳ [تنسيق التباعد] انتظار أمان مخصص للبوت الثاني لمدة ${Math.round(initialOffsetDelay / 1000 / 60)} دقائق لضمان عدم التزامن...`, 'info');
    ...
    // 🛠️ خيارات تشغيل متصفح آمنة خالية من أَعلام البوتات
    const launchOptions = {
        headless: true,
        args: [
            '--no-sandbox',
    ...
    await logToDashboard(`⚡ تم تشغيل المتصفح بضبط أمان خاص لمنع Checkpoint`, 'info');
    ...
    try {
        ...
        while (true) {
            // 🛑 1. فحص كروت الإيقاف الفورية المخصصة لـ bot2 حصراً
            ...
            if (freshData.skip_current_group === true) {
                ...
                let currentBotGroup = freshData.bot2_group;
            ...
            if (groups.length === 0 && !botGroup) {
                ...
                const hasOtherBotGroups = checkAllBots && (checkAllBots.bot1_group || checkAllBots.bot3_group);
                ...
                if (!hasOtherBotGroups) {
                    ...
                    await logToDashboard(`🎉 اكتملت جميع المجموعات المخصصة للبوت (2)! ينتهي البوت الثاني مع استمرار البوتات الأخرى...`, 'success');
                }
            ...
            try {
                ...
                if (isCheckpoint) {
                    ...
                    await supabase.from('publish_queue').update({ bot2_group: null, ai_final_text2: null }).eq('id', initialPostData.id);
...
async function startBot2Engine() {
    ...
    while (true) {
        try {
            // 🛑 فحص كرت الإيقاف في المحرك الرئيسي لـ bot2 حصراً
            const { data: counterStatus } = await supabase
                .from('bot_counters')
                .select('status')
                .eq('bot_name', BOT_ID)
            ...
            if (counterStatus && ['IDLE', 'STOPPED', 'PAUSED'].includes(counterStatus.status)) {
                await forceKillProcess('تم رصد حالة الإيقاف في المحرك الرئيسي للبوت الثاني');
            }
